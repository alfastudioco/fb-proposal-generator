...
`POST /api/import-document` — body `{ fileBase64, mediaType }` (`mediaType` one of `application/pdf`, `image/jpeg`, `image/png`, `image/webp`) — a PDF or photo of any invoice, estimate, or proposal (not limited to any particular software's format). Returns `{ client, proposalNum, sections, notes, clientSupplied, matches }` — a full new proposal drafted from the document's line items, preserving the original pricing, plus `matches` (candidate existing `fbpg_clients` rows, same shape `/api/extract-client` returns) so a repeat client can be linked instead of duplicated. Powers "Import Invoice/Proposal", which wholesale-replaces the current form (`loadImportedProposal`) — a "start a new proposal" action, not a merge.
...
- **Chat edits return a patch, not the whole proposal**: the model returns only changed/added/removed rooms (by index) and changed fields, and `api/generate-full-proposal.js`'s `applyEditPatch` merges that into the proposal the browser sent, so the response shape above is unchanged. An instruction that rewrites many rooms at once can still hit the output limit — split it into smaller steps.
- **Show total price only**: one proposal-wide toggle in the Rooms panel that drops the price cell from every room banner in both the .docx and the PDF, so only the total investment shows; rooms still count toward the total. It's saved as `hidePrice` on every section (inside the `sections` jsonb, so no migration); a proposal with any hidden room loads with the toggle on. The chat can set it too (`hideAllSectionPrices` in the edit tool), and the edit tool also returns a `message` shown in the panel -- what it changed, or why the format can't do what was asked.
- **Save as New Version** (shown when editing a saved proposal) inserts a new `fbpg_proposals` row instead of updating the loaded one, bumping the proposal number `1901` → `1901-B` → `1901-C` unless you've already changed it, and not carrying over deposit/balance tracking. After that, Save Changes updates the new version.
- **Applying a chat edit clears each room's "describe the project" textarea** (the AI-scope-generator input field), since chat-Apply repopulates all rooms fresh the same way loading a saved proposal does — this is not sent to the server, just a UI field, but worth knowing.
