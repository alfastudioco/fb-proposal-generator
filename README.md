...
- **Hiding room prices**: each section has an optional `hidePrice` flag (stored inside the `sections` jsonb, so no migration) that drops the price cell from that room's banner in both the .docx and the PDF; the room still counts toward the total. "Show total only" in the Rooms panel sets it on every room, and the chat can set it too (`hideAllSectionPrices` in the edit tool). The chat's edit tool also returns a `message` shown in the panel -- what it changed, or why the format can't do what was asked.
